...
use crate::{add_edge, add_node, add_node_with_edge, debug_info_red, get_ast_from_symidx, node};
use anyhow::Result;
use super::{cfg_node::InstrList, et_node::{EtEdgeType, EtNode, EtNodeType, EtTree}, nhwc_instr::{ArithOp, InstrSlab, NhwcInstr}, scope_node::ScopeTree, symtab::{self, SymIdx, SymTab}};
...
pub fn process_trans_et(transed_symidx:&SymIdx,child_et_map:&mut HashMap<SymIdx,u32>,scope_tree:&ScopeTree,to_trans_ast:u32,lhs:&SymIdx,instr_et:&mut EtTree){
    ...
    if transed_symidx.is_literal(){
        EtNodeType::new_literal(to_trans_ast, lhs.clone()).into()
    }else{
        EtNodeType::new_symbol(to_trans_ast, lhs.clone(), super::et_node::DeclOrDefOrUse::Def).into()
    };
    let to_trans_et_node = add_node!({to_trans_et_struct.clone()} to instr_et);
    if let Some(&transed_et_node) = child_et_map.get(transed_symidx){
        add_edge!({EtEdgeType::Direct.into()} from to_trans_et_node to transed_et_node in instr_et);
    }else{
        let transed_et_node = add_node_with_edge!({EtNodeType::new_symbol(transed_ast, transed_symidx.clone(), super::et_node::DeclOrDefOrUse::Use).into()} with_edge {EtEdgeType::Direct.into()} from to_trans_et_node in instr_et);
        to_trans_et_struct.equivalent_symidx_vec.push(lhs.clone());
        child_et_map.insert(transed_symidx.clone(), transed_et_node);
    }
    child_et_map.insert(lhs.clone(), to_trans_et_node);
}
...
pub fn parse_instr_list_to_et(instrs:&InstrList,instr_et:&mut EtTree,stmtab:&SymTab,child_et_map:&mut HashMap<SymIdx,u32>,scope_tree:&ScopeTree,instr_slab:&InstrSlab<NhwcInstr>) -> Result<()>{
    for &instr in instrs.iter(){
        ...
        match &instr.instr_type{
            ...
            super::nhwc_instr::NhwcInstrType::SimpleAssign { lhs, rhs, vartype:_ } => {
                ...
                if let Some(&rhs_et_node) = child_et_map.get(rhs){
                    ...
                    assign_et_struct.clone().equivalent_symidx_vec.push(lhs.clone());
                    child_et_map.insert(lhs.clone(), rhs_et_node);
                }else{
                    if rhs.is_literal(){
                        let mut literal_et_struct:EtNode = EtNodeType::new_literal(rhs_ast, rhs.clone()).into();
                        let literal_et_node = add_node!({literal_et_struct.clone()} to instr_et);
                        literal_et_struct.equivalent_symidx_vec.push(lhs.clone());
            ...
            super::nhwc_instr::NhwcInstrType::TranType { lhs, op } => {
                let trans_ast = get_ast_from_symidx!(find lhs with scope_tree);
                match op{
                    super::nhwc_instr::Trans::Fptosi { float_symidx } => {
                        process_trans_et(float_symidx, child_et_map,scope_tree, trans_ast, lhs, instr_et);
                    },
                    super::nhwc_instr::Trans::Sitofp { int_symidx } => {
                        process_trans_et(int_symidx, child_et_map,scope_tree, trans_ast, lhs, instr_et);
                    },
                    super::nhwc_instr::Trans::Zext { bool_symidx } => {
                        process_trans_et(bool_symidx, child_et_map,scope_tree, trans_ast, lhs, instr_et);
                    },
                    super::nhwc_instr::Trans::Bitcast { rptr_symidx, rptr_type:_, lptr_type:_ } => {
                        process_trans_et(rptr_symidx, child_et_map,scope_tree, trans_ast, lhs, instr_et);
                    },
                }
            },
