...
fn parse_stmt2nhwc(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,symtab:&mut SymTab,scope_tree:&ScopeTree,et_tree:&mut EtTree,scope_node:u32,ast_stmt_node:u32,cfg_bb:u32,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_g:&mut Option<&mut SymTabGraph>)->Result<()>{
        ...
        for et_node in et_nodes{
            ...
            match et_struct{
                    ...
                    if let Some(_) = direct_child_node!(at et_node in et_tree ret option){
                        match op{
                            crate::toolkit::et_node::ExprOp::Assign => {
                                ...
                                // 后序遍历 右边
                                let value_symidx = process_et(ast_tree, cfg_graph, et_tree, scope_tree, symtab, op_values[1], stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                let value_type = find!(field TYPE:Type at value_symidx in symtab debug symtab_graph symtab_g).unwrap().clone();
                                // 后序遍历 左边
                                let var_symidx = process_et(ast_tree, cfg_graph, et_tree, scope_tree, symtab, op_values[0], stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                ...
                                let new_value_symidx = force_trans_type(cfg_graph, symtab, &var_type, &value_type, &value_symidx, stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                            ...
                            crate::toolkit::et_node::ExprOp::LPlusPlus => {
                                if let Some(symbol_node) = direct_child_node!(at et_node in et_tree ret option){
                                    let (_,_) = process_self_increment(ast_tree, cfg_graph, et_tree, scope_tree, symtab,  symbol_node, stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                }else{
                            ...
                            crate::toolkit::et_node::ExprOp::MulAssign => {
                                ...
                                let (mul_tmp_symidx,var_symidx,value_symidx,mul_tmp_type,var_type) = process_arithop(ast_tree, cfg_graph, et_tree, scope_tree, symtab, et_node, stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                ...
                                //赋值instr
                                let assign_tmp_symidx = force_trans_type(cfg_graph, symtab, &var_type, &mul_tmp_type,&mul_tmp_symidx ,stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                let tmp_var_symidx = add_symbol!({Symbol::new(stmt_parent_scope,format!("temp_{}",counter))} with field TYPE:{var_type.clone()} to symtab debug symtab_g);
                                *counter += 1;
                            ...
                            crate::toolkit::et_node::ExprOp::DivAssign => {
                                //将/=取下面的两个子树，视作算术运算符进行自动类型转换
                                let (div_tmp_symidx,var_symidx,value_symidx,div_tmp_type,var_type) = process_arithop(ast_tree, cfg_graph, et_tree, scope_tree, symtab, et_node, stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                ...
                                //赋值instr
                                let assign_tmp_symidx = force_trans_type(cfg_graph, symtab, &var_type, &div_tmp_type,&div_tmp_symidx ,stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                let tmp_var_symidx = add_symbol!({Symbol::new(stmt_parent_scope,format!("temp_{}",counter))} with field TYPE:{var_type.clone()} to symtab debug symtab_g);
                                *counter += 1;
                                let assign_instr = InstrType::new_assign(tmp_var_symidx,assign_tmp_symidx ).to_instr();
                                push_instr!(assign_instr to cfg_bb in cfg_graph slab instr_slab);
                            ...
                            crate::toolkit::et_node::ExprOp::PlusAssign => {
                                //将+=取下面的两个子树，视作算术运算符进行自动类型转换
                                let (add_tmp_symidx,var_symidx,value_symidx,add_tmp_type,var_type) = process_arithop(ast_tree, cfg_graph, et_tree, scope_tree, symtab, et_node, stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                ...
                                //赋值instr
                                let assign_tmp_symidx = force_trans_type(cfg_graph, symtab, &var_type, &add_tmp_type,&add_tmp_symidx ,stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                let tmp_var_symidx = add_symbol!({Symbol::new(stmt_parent_scope,format!("temp_{}",counter))} with field TYPE:{var_type.clone()} to symtab debug symtab_g);
                                *counter += 1;
                                let assign_instr = InstrType::new_assign(tmp_var_symidx,assign_tmp_symidx ).to_instr();
                                push_instr!(assign_instr to cfg_bb in cfg_graph slab instr_slab);
                            ...
                            crate::toolkit::et_node::ExprOp::MinusAssign => {
                                //将-=取下面的两个子树，视作算术运算符进行自动类型转换
                                let (sub_tmp_symidx,var_symidx,value_symidx,sub_tmp_type,var_type) = process_arithop(ast_tree, cfg_graph, et_tree, scope_tree, symtab,  et_node, stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                ...
                                //赋值instr
                                let assign_tmp_symidx = force_trans_type(cfg_graph, symtab, &var_type, &sub_tmp_type,&sub_tmp_symidx ,stmt_parent_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                                let tmp_var_symidx = add_symbol!({Symbol::new(stmt_parent_scope,format!("temp_{}",counter))} with field TYPE:{var_type.clone()} to symtab debug symtab_g);
                                *counter += 1;
                                let assign_instr = InstrType::new_assign(tmp_var_symidx,assign_tmp_symidx ).to_instr();
                                push_instr!(assign_instr to cfg_bb in cfg_graph slab instr_slab);
                ...
                EtNodeType::Symbol { sym_idx:_, ast_node:_, text, def_or_use } => {
                    ...
                    let symbol_symidx = process_symbol(ast_tree,scope_tree, symtab, &def_or_use, &text, stmt_parent_scope,  symtab_g)?;
                ...
                _ =>{
                    return Err(anyhow!("{}这里不应该为sep类型",et_node))
                }
            }
        }
        Ok(())
...
fn parse_expr(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,symtab:&mut SymTab,scope_tree:&ScopeTree,et_tree:&mut EtTree,expr_scope:u32,expr_node:u32,cfg_node:u32,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_g:&mut Option<&mut SymTabGraph>)->Result<()>{
                ...
                match op{
                    super::et_node::ExprOp::LogicalOr => {
                        if let Some(_) = direct_child_node!(at op_et in et_tree ret option){
                            let (tmp_var_symidx, l_symidx, r_symidx) = process_logicop(ast_tree, cfg_graph, et_tree, scope_tree, symtab,  op_et, expr_parent_scope, cfg_node, counter, instr_slab, symtab_g)?;
                    ...
                    super::et_node::ExprOp::LogicalNot => {
                        if let Some(next_node) = direct_child_node!(at op_et in et_tree ret option){
                            //取操作数的symidx和type
                            let symbol_symidx = process_et(ast_tree,cfg_graph, et_tree, scope_tree, symtab, next_node, expr_parent_scope, cfg_node, counter, instr_slab, symtab_g)?;
                            ...
                            let num2bool_tmp_symidx = add_symbol!({Symbol::new(expr_parent_scope,format!("temp_{}",counter))} with field TYPE:{Type::I1} to symtab debug symtab_g);
                            *counter += 1;
                            match symbol_type {
                                Type::F32 => {
                                    let fzero_symidx = process_constant(symtab, &"0.0".to_string(), symtab_g)?;
                                ...
                                Type::I32 => {
                                    let izero_symidx = process_constant(symtab, &"0".to_string(), symtab_g)?;
                                ...
                                Type::I1 => { },
                                _ => {
                                    return Err(anyhow!("类型{:?}不能进行逻辑运算",symbol_type))
                                }
                            }
                            let tmp_var_symidx = add_symbol!({Symbol::new(expr_parent_scope,format!("temp_{}",counter))} with field TYPE:{Type::I1} to symtab debug symtab_g);
                            *counter += 1;
                    ...
                    super::et_node::ExprOp::Eq => {
                        if let Some(_) = direct_child_node!(at op_et in et_tree ret option){
                            let (tmp_var_symidx, l_symidx, r_symidx,var_type,_) = process_arithop(ast_tree, cfg_graph, et_tree, scope_tree, symtab, op_et,  expr_parent_scope, cfg_node, counter, instr_slab, symtab_g)?;
                    ...
                    _ => {
                        return Err(anyhow!("条件表达式不能返回bool类型{}",ast_node))
                    }
                }
            },
...
fn parse_bb2nhwc(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,scope_tree:&ScopeTree,et_tree:&mut EtTree,symtab:&mut SymTab,ast2scope:&HashMap<u32,u32>,ast_nodes:Vec<u32>,cfg_bb:u32,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_g:&mut Option<&mut SymTabGraph>)->Result<()>{
    for astnode in ast_nodes{
        match(rule_id!(at astnode in ast_tree),astnode){
            ...
            (_,_)=>{
                return Err(anyhow!("bb中未知RULE，{}不是expr或stmt",astnode))
            }
        }
    }
    Ok(())
}
fn parse_whileloop2nhwc(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,scope_tree:&ScopeTree,et_tree:&mut EtTree,symtab:&mut SymTab,ast2scope:&HashMap<u32,u32>,ast_expr_node:u32,cfg_whileloop:u32,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_graph:&mut Option<&mut SymTabGraph>)->Result<()>{
    match(rule_id!(at ast_expr_node in ast_tree),ast_expr_node){
        ...
        (_,_)=>{
            return Err(anyhow!("whileloop中未知RULE,{}不是expr或stmt",ast_expr_node))
        }
    }
    Ok(())
}
fn parse_forloop2nhwc(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,scope_tree:&ScopeTree,et_tree:&mut EtTree,symtab:&mut SymTab,ast2scope:&HashMap<u32,u32>,ast_before_node:u32,ast_mid_node:u32,ast_after_node:u32,cfg_forloop:u32,op_body_head_and_tail:Option<(u32,u32)>,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_graph:&mut Option<&mut SymTabGraph>)->Result<()>{
    match rule_id!(at ast_before_node in ast_tree){
        RULE_forBeforeExpression=>{
            ...
            if let Some(before_scope) = ast2scope.get(&ast_before_node){
                ...
                parse_declaration2nhwc(ast_tree, cfg_graph, symtab, scope_tree, et_tree, *before_scope, before_ast, cfg_forloop, counter, instr_slab, symtab_graph)?
            }else{
                return Err(anyhow!("找不到astnode的scope"))
            }
        },
        _=>{
            return Err(anyhow!("forloop中未知RULE,{}不是或forBeforeExpression",ast_before_node))
        }
    }
    match rule_id!(at ast_mid_node in ast_tree){
        ...
        _=>{
            return Err(anyhow!("forloop中未知RULE,{}不是或forMidExpression",ast_after_node))
        }
    }
    match rule_id!(at ast_after_node in ast_tree){
        RULE_forAfterExpression=>{
            ...
            if let Some(after_scope) = ast2scope.get(&ast_after_node){
                parse_stmt2nhwc(ast_tree, cfg_graph, symtab, scope_tree, et_tree, *after_scope,ast_after_node, cfg_forloop, counter, instr_slab, symtab_graph);
            }else{
                return Err(anyhow!("找不到astnode的scope"))
            }
        },
        _=>{
            return Err(anyhow!("forloop中未知RULE,{}不是或forAfterExpression",ast_after_node))
        }
    }
    Ok(())
}
...
fn process_symbol(ast_tree:&AstTree,scope_tree:&ScopeTree,symtab:&mut SymTab,def_or_use:&Def_Or_Use,symbol_name:&String,scope_node:u32, symtab_graph:&mut Option<&mut SymTabGraph>)->Result<SymIdx>{   
...
fn force_trans_type(cfg_graph:&mut CfgGraph,symtab:&mut SymTab,to_trans_type:&Type,transed_type:&Type,transed_symidx:&SymIdx,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>)->Result<SymIdx>{
    match(transed_type,to_trans_type){
        ...
        _ => {
            return Err(anyhow!("该类型不支持强制转化"))
        }
    }
}
///算数运算符自动类型转换，返回转换后两个操作符的symidx
fn autotrans_arith_type(cfg_graph:&mut CfgGraph,symtab:&mut SymTab,l_type:&Type,l_symidx:&SymIdx,r_type:&Type,r_symidx:&SymIdx,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>)->Result<(SymIdx,SymIdx)>{
    ...
    match(l_type.clone(),r_type.clone()){
        ...
        _ =>{
            return Err(anyhow!("操作数类型不能参与运算"))
        },
    }
}
///逻辑运算符自动类型转换，返回转换后的两个操作符的symidx
fn autotrans_logic_type(cfg_graph:&mut CfgGraph,symtab:&mut SymTab,l_type:&Type,l_symidx:&SymIdx,r_type:&Type,r_symidx:&SymIdx,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>)->Result<(SymIdx,SymIdx)>{
    //adapt函数会去除掉不能进行运算的类型情况
    match(l_type.clone(),r_type.clone()){
        (Type::I32,Type::F32) =>{
            //i32转i1
        ...
        _ =>{
            return Err(anyhow!("操作数无法进行逻辑计算"))
        },
    }
}
///处理自增运算符，不分左右,统一返回增加instr和赋值instr
fn process_self_increment(ast_tree:&AstTree,cfg_graph:&mut CfgGraph,et_tree:&EtTree,scope_tree:&ScopeTree,symtab:&mut SymTab,op_et_node:u32,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>) -> Result<(SymIdx,SymIdx)>{
    //取自增运算符下的symidx和type
    ...
    *counter += 1;
    match var_type {
        ...
        _ => {
            return Err(anyhow!("自增自减操作数不是数字类型的"))
        },
    }
    //自增后赋值的instr
...
///处理自减运算符，不分左右
fn process_self_attennuation(ast_tree:&AstTree,cfg_graph:&mut CfgGraph,et_tree:&EtTree,scope_tree:&ScopeTree,symtab:&mut SymTab,op_et_node:u32,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>) -> Result<(SymIdx,SymIdx)>{
    //取操作数的symidx和type
    let var_symidx = process_et(ast_tree,cfg_graph,et_tree, scope_tree, symtab,  op_et_node, scope_node,  cfg_bb, counter, instr_slab, symtab_graph)?;
    let var_type = find!(field TYPE:Type at var_symidx in symtab debug symtab_graph symtab_graph).unwrap().clone();
    //读取变量的instr
...
fn process_arithop(ast_tree:&AstTree,cfg_graph:&mut CfgGraph,et_tree:&EtTree,scope_tree:&ScopeTree,symtab:&mut SymTab,root_et_node:u32,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>)->Result<(SymIdx,SymIdx,SymIdx,Type,Type)>{
...
fn process_logicop(ast_tree:&AstTree,cfg_graph:&mut CfgGraph,et_tree:&EtTree,scope_tree:&ScopeTree,symtab:&mut SymTab,op_et_node:u32,scope_node:u32,cfg_bb:u32,counter:&mut u32,instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>)->Result<(SymIdx,SymIdx,SymIdx)>{
...
fn process_et(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,et_tree:&EtTree,scope_tree:&ScopeTree,symtab:&mut SymTab,et_node:u32,scope_node:u32,cfg_bb:u32,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_graph:&mut Option<&mut SymTabGraph>)->Result<SymIdx>{
    ...
    match nake_et{
        EtNodeType::Operator { op, ast_node:_, text:_ } => {
            match op{
                super::et_node::ExprOp::Mul => {
                    if let Some(_) = direct_child_node!(at et_node in et_tree ret option){
                        let (tmp_var_symidx, l_symidx, r_symidx,var_type,_) = process_arithop(ast_tree, cfg_graph, et_tree, scope_tree, symtab,  et_node,  scope_node, cfg_bb, counter, instr_slab, symtab_graph)?;
                ...
                super::et_node::ExprOp::LPlusPlus => {
                    if let Some(symbol_node) = direct_child_node!(at et_node in et_tree ret option){
                        let (tmp_addvar_symidx,tmp_loadvar_symidx) = process_self_increment(ast_tree, cfg_graph, et_tree, scope_tree, symtab, symbol_node, scope_node, cfg_bb, counter, instr_slab, symtab_graph)?;
                ...
                _ => {
                    return Err(anyhow!("表达式{:?}内不应存在带有显性赋值性质的操作符",op))
                },
            }
        },
        ...
        _=>{
            return Err(anyhow!("{}不应出现sep类型的et",et_node))
        }
    }
}

///定义变量的decl转为ir，并通过et查找元素是否合法
fn parse_declaration2nhwc(ast_tree:&AstTree,cfg_graph: &mut CfgGraph,symtab:&mut SymTab,scope_tree:&ScopeTree,et_tree:&mut EtTree,decl_scope:u32,ast_decl_node:u32,cfg_bb:u32,counter:&mut u32, instr_slab:&mut InstrSlab, symtab_g:&mut Option<&mut SymTabGraph>)->Result<()>{
            ...
            match etnode{
                EtNodeType::Operator { op:_, ast_node:_, text:_ } => {
                    if let Some(_) = direct_child_node!(at detail_et in et_tree ret option){
                        ...
                        let var_symidx = process_et(ast_tree, cfg_graph, et_tree, scope_tree, symtab,  op_values[0], decl_prt_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                        let value_symidx = process_et(ast_tree, cfg_graph, et_tree, scope_tree, symtab, op_values[1], decl_prt_scope, cfg_bb, counter, instr_slab, symtab_g)?;
                        let value_type = find!(field TYPE:Type at value_symidx in symtab debug symtab_graph symtab_g).unwrap().clone();
                ...
                _ =>{
                    return Err(anyhow!("{}这里不应该为sep类型",detail_et))
                }
            }
        }
        Ok(())
...
fn parse_func2nhwc(ast_tree:&AstTree,cfg_graph:&mut CfgGraph,symtab:&mut SymTab,ast2scope:&HashMap<u32,u32>,ast_fun:u32,ast_funsign:u32,cfg_entry:u32, instr_slab:&mut InstrSlab,symtab_graph:&mut Option<&mut SymTabGraph>)->Result<()>{
...
pub fn parse_cfg_into_nhwc_cfg(cfg_graph:&mut CfgGraph, scope_tree:&mut ScopeTree, ast_tree:&mut AstTree,symtab:&mut SymTab, et_tree:&mut EtTree, ast2scope:&mut HashMap<u32,u32> , mut counter:u32, instr_slab:&mut InstrSlab, symtab_g:&mut Option<&mut SymTabGraph>)->Result<()>{
    ...
    for cfg_entry in cfg_funcs.clone(){
        match node!(at cfg_entry in cfg_graph).cfg_type {
            ...
            _ => {
                return Err(anyhow!("entry不是函数签名,cfg出错"))
            }
        }   
    }
