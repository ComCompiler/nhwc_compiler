...
pub fn eval_et(et_tree:&mut EtTree , any_et_node:u32 ) ->SymbolIndex {
    let mut value = SymbolIndex::new(0,0.to_string());
    ...
    // let value = match node!(at Operator_node in et_tree){
        ...
        EtNakedNode::Operator { ast_node, text, op } =>{
            ...
            let mut sub_nodes_sym_idx: Vec<SymbolIndex> = Vec::new();
