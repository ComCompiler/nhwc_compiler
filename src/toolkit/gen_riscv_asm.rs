            ...
            for &instr in node!(at cfg_node in cfg_graph).iter_all_instrs() {
                ...
                match &instr_struct.instr_type{
                    ...
                    NhwcInstrType::DefineVar { var_symidx, vartype: _, op_value } => {
                        match op_value{
                            Some(value_symidx) => {
                                let val_reg = load_sym_or_imm(&mut asm_sect, value_symidx, Register::new_s(1), src_symtab)?;
                    ...
                    NhwcInstrType::Store { value_symidx, value_ty: _, ptr_symidx, ptr_ty: _ } => {
                        let val_reg = load_sym_or_imm(&mut asm_sect, value_symidx, Register::new_s(2), src_symtab)?;
                    ...
                    NhwcInstrType::Arith { lhs, rhs } => {
                        ...
                        match rhs{
                            super::nhwc_instr::ArithOp::Add { a, b, vartype: _ } => {
                                ...
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm({Arithmetic::new_add}(Register::new_s(3),val_reg1,val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::Mul { a, b, vartype: _ } => {
                                ...
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm(Arithmetic::new_mul(Register::new_s(3),val_reg1,val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::Div { a, b, vartype: _ } => {
                                ...
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm(Arithmetic::new_div(Register::new_s(3),val_reg1,val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::Sub { a, b, vartype: _ } => {
                                ...
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm(Arithmetic::new_sub(Register::new_s(3), val_reg1, val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::Mod { a, b, vartype: _ } => {
                                ...
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm({Arithmetic::new_rem} (Register::new_s(3),val_reg1,val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::Icmp { plan, a, b, vartype: _ } => {
                                match plan{
                                    super::nhwc_instr::IcmpPlan::Eq => {
                                        ...
                                        let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                        let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                        asm_sect.asm({Logical::new_xor } (Register::new_s(3),val_reg1,val_reg2).into());
                                    ...
                                    super::nhwc_instr::IcmpPlan::Ugt => {
                                        let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                        let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                        asm_sect.asm({Compare::new_sltu } (Register::new_s(3), val_reg2, val_reg1).into());
                                    },
                                    ...
                                    super::nhwc_instr::IcmpPlan::Ult => {
                                        let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                        let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                        asm_sect.asm({Compare::new_sltu } (Register::new_s(3), val_reg1, val_reg2).into());
                                    },
                                    ...
                                    super::nhwc_instr::IcmpPlan::Sgt => {
                                        let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                        let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                        asm_sect.asm({Compare::new_slt } (Register::new_s(3), val_reg2, val_reg1).into());
                                    },
                                    ...
                                    super::nhwc_instr::IcmpPlan::Slt => {
                                        let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                        let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                        asm_sect.asm({Compare::new_slt } (Register::new_s(3), val_reg1, val_reg2).into());
                                    },
                            ...
                            super::nhwc_instr::ArithOp::LogicAnd { a, b, vartype: _ } => {
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm({Logical::new_and } (Register::new_s(3),val_reg1,val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::LogicOr { a, b, vartype: _ } => {
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                let val_reg2 = load_sym_or_imm(&mut asm_sect, b, Register::new_s(2), src_symtab)?;
                                asm_sect.asm({Logical::new_or } (Register::new_s(3),val_reg1,val_reg2).into());
                            },
                            super::nhwc_instr::ArithOp::LogicNot { a, vartype: _ } => {
                                let val_reg1 = load_sym_or_imm(&mut asm_sect, a, Register::new_s(1), src_symtab)?;
                                asm_sect.asm({Logical::new_xori } (Register::new_s(3), val_reg1, Imm::from_offset(-1)).into());
                            },
                        }
                        // store reg3 to lhs
                        store_sym(&mut asm_sect, lhs, Register::new_s(3), src_symtab)?;
                    ...
                    NhwcInstrType::SimpleAssign { lhs, rhs } => {
                        let val_reg1 = load_sym_or_imm(&mut asm_sect, rhs, Register::new_s(3), src_symtab)?;
                    ...
                    NhwcInstrType::Jump { jump_op } => {
                        match jump_op{
                            ...
                            super::nhwc_instr::JumpOp::Br { cond, t1: _, t2 } => {
                                ...
                                asm_sect.asm(PseudoInstr::new_bnez(Register::new_s(1),Imm::new_jump_label(t2.clone())).into());
                    ...
                    NhwcInstrType::TranType { lhs, op } => {
                        match op{
                            super::nhwc_instr::Trans::Fptosi { float_symidx } => {
                                let val_reg = load_sym_or_imm(&mut asm_sect, float_symidx, Register::new_s(1), src_symtab)?;
                                asm_sect.asm(Trans::new_fcvt_w_s(Register::new_s(1),val_reg).into());
                            },
                            super::nhwc_instr::Trans::Sitofp { int_symidx } => {
                                let val_reg = load_sym_or_imm(&mut asm_sect, int_symidx, Register::new_s(1), src_symtab)?;
                                asm_sect.asm(Trans::new_fcvt_s_w(Register::new_fs(1),val_reg).into());
                            },
                            super::nhwc_instr::Trans::Zext { bool_symidx } => {
                                //b->i
                                let val_reg = load_sym_or_imm(&mut asm_sect, bool_symidx, Register::new_s(1), src_symtab)?;
                                asm_sect.asm({Logical::new_andi} (Register::new_s(3),val_reg,Imm::from_offset(1)).into());
                            },
...
pub fn load_sym_or_imm(asm_sect:&mut AsmSection,symidx:&SymIdx,reg:Register,src_symtab:&SymTab) -> Result<Register>{
        ...
        match !(src_symtab.has_symbol(symidx) && !*src_symtab.get_symbol(symidx)?.get_is_literal()?){
            true => {
                ...
                match Type::new_from_const(&symidx.symbol_name) {
                    Type::F32 => {
                        ...
                        asm_sect.asm(PseudoInstr::new_fmv_s(reg.to_f_reg(), reg.clone()).into());
                        Ok(reg.to_f_reg())
                    },
                    _ => {
                        // i32 ...
            ...
            false => {
                ...
                match src_symtab.get_symbol(symidx)?.get_type()?{
                    Type::F32 => {
                        asm_sect.asm(Loads::new(size,reg.clone().to_f_reg(), Register::SP {  }, symidx_offset2sp as isize, true)?.into());
                        Ok(reg.to_f_reg())
                    },
                    _ => {
...
pub fn store_sym(asm_sect:&mut AsmSection, symidx:&SymIdx,value_reg:Register,src_symtab:&SymTab) -> Result<()>{
    let &symidx_offset2sp = src_symtab.get_symbol(symidx)?.get_mem_offset2sp()?;
    let size = src_symtab.get_symbol(symidx)?.get_type()?.get_ele_size()?;
    if value_reg.is_f_reg() ^ src_symtab.get_symbol(symidx)?.get_type()?.is_f_32(){
        return Err(anyhow!("can't store symidx {:?}:{:?} to register {:?}",symidx,src_symtab.get_symbol(symidx)?.get_type()?,value_reg))
    }
    asm_sect.asm(Stores::new( size,value_reg.clone(),Register::new_sp(), symidx_offset2sp as isize, value_reg.is_f_reg())?.into());
    Ok(())
}
/// reg value -> M[ptr_sym]
...
pub fn store_from_ptr(asm_sect:&mut AsmSection,ptr_symidx:&SymIdx, ptr_reg:Register, value_reg:Register ,src_symtab:&SymTab)-> Result<()>{
    ...
    if value_reg.is_f_reg() ^ src_symtab.get_symbol(ptr_symidx)?.get_type()?.to_deref_type()?.is_f_32(){
    ...
    asm_sect.asm(Stores::new( size,value_reg.clone(),ptr_reg, 0, value_reg.is_f_reg())?.into());
    Ok(())
}
...
pub fn load_from_ptr(asm_sect:&mut AsmSection,ptr_symidx:&SymIdx,ptr_reg:Register, value_reg:Register ,src_symtab:&SymTab) ->Result<Register>{
    let actual_ptr_reg = load_sym_or_imm(asm_sect, ptr_symidx, ptr_reg.clone(), src_symtab)?;
    ...
    if is_f32{
        asm_sect.asm(Loads::new(size, value_reg.to_f_reg(), actual_ptr_reg.clone(), 0, true)?.into());
        Ok(value_reg.to_f_reg())
    }else{
        asm_sect.asm(Loads::new(size, value_reg.clone(), actual_ptr_reg.clone(), 0, false)?.into());
        Ok(value_reg)
    }
}
