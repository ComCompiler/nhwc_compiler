...
pub fn parse_as_ast_graph(code :String,debug_info:bool)-> AstTree{
    let mut g: Graph<AstNode,(), petgraph::Directed> = Graph::new();
    // 由于 antlr 已经生成了一个 AST 树 但我们需要的是 petgraph 类型，因此我们需要重新遍历一次这个树，生成 petgraph的
    {
    let listener = RuleOnlyListener{
        st: (Vec::<usize>::new(),false,&mut g),
        enter_rule_f:Box::new(|ctx,s|{
            let (node_count_under_depth,is_last_wrap_drop,g) = s;
            let node_id = g.add_node(AstNode::new(ctx.get_rule_index(),ctx.get_text()));
