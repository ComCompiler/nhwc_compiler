    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if !self.is_terminal {
            write!(
                f,
                "{} {} {}",
                crate::antlr_parser::cparser::ruleNames[self.rule_id],
                self.text,
                self.node_index
            )
        } else {
            write!(
                f,
                "{} {} {}",
                crate::antlr_parser::clexer::_SYMBOLIC_NAMES[self.rule_id].unwrap(),
                self.text,
                self.node_index
            )
        }
...
pub fn find_dfs_rule_ast<'a>(
    ast_tree: &'a AstTree,
    start: u32,
    target_rule_id: usize,
...
// 这个函数返回
pub fn find_neighbors_term_ast<'a>(
    ast_tree: &'a AstTree,
    start: u32,
    op_target_term_id: Option<isize>,
) -> impl Iterator<Item = u32> + 'a {
    ...
    ns.map(|x| x.index() as u32).filter(move |x| {
        (op_target_term_id.is_none()
            || (ast_tree.node_weight(NodeIndex::from(*x)).unwrap().rule_id == {
                let a: usize = (op_target_term_id.unwrap() as u32)
                    .try_into()
                    .expect("isize to usize 失败");
                a
            }))
            && ast_tree
                .node_weight(NodeIndex::from(*x))
                .unwrap()
                .is_terminal
                == true
    })
}
pub fn find_neighbors_rule_ast<'a>(
    ast_tree: &'a AstTree,
    start: u32,
    op_target_rule_id: Option<usize>,
) -> impl Iterator<Item = u32> + 'a {
    let ns = ast_tree.neighbors(NodeIndex::from(start));
    ns.map(|x| x.index() as u32).filter(move |x| {
        (op_target_rule_id.is_none()
            || (ast_tree.node_weight(NodeIndex::from(*x)).unwrap().rule_id
                == op_target_rule_id.unwrap()))
            && ast_tree
                .node_weight(NodeIndex::from(*x))
                .unwrap()
                .is_terminal
