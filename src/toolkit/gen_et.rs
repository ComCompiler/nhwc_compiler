...
use super::et_node::{self, Def_Or_Use, EtNodeType, EtNode, EtTree};
...
fn process_equality_expr(et_tree: &mut EtTree, ast_tree:&AstTree, scope_tree: &ScopeTree, equality_expr_node: u32, scope_node: u32, parent_et_node: u32) {
    ...
    let get_expr_node_of_op_node = |op_node_index|{
        ...
        match term_id!(at op_node in ast_tree) {
            Equal=>{
                EtNodeType::new_op_equal(equality_expr_node).to_et_node()
            }
            NotEqual=>{
                EtNodeType::new_op_not_equal( equality_expr_node).to_et_node()
            }
            _=>{
...
fn process_relational_expr(et_tree: &mut EtTree, ast_tree: &AstTree, scope_tree: &ScopeTree, relational_expr_node: u32, scope_node: u32, parent_et_node: u32) {
    ...
    let get_expr_node_of_op_node = |op_node_index| {
        ...
        match term_id!(at op_node in ast_tree) {
            Less => {
                EtNodeType::new_op_less_than(relational_expr_node).to_et_node()
            },
            Greater=> {
                EtNodeType::new_op_greater_than( relational_expr_node).to_et_node()
            },
            LessEqual => {
                EtNodeType::new_op_less_than_or_equal(relational_expr_node).to_et_node()
            },
            GreaterEqual => {
...
fn process_shift_expr(et_tree: &mut EtTree, ast_tree: &AstTree, scope_tree: &ScopeTree, shift_expr_node: u32, scope_node: u32, parent_et_node: u32) {
    ...
    let get_expr_node_of_op_node = |op_node_index| {
        ...
        match term_id!(at op_node in ast_tree) {
            LeftShift => {
                EtNodeType::new_op_left_shift(shift_expr_node).to_et_node()
            },
            RightShift => {
                EtNodeType::new_op_right_shift( shift_expr_node).to_et_node()
            },
...
fn process_additive_expr(et_tree: &mut EtTree, ast_tree: &AstTree, scope_tree: &ScopeTree, additive_expr_node: u32, scope_node: u32, parent_et_node: u32) {
    ...
    let get_expr_node_of_op_node = |op_node_index| {
        ...
        match term_id!(at op_node in ast_tree) {
            Plus => {
                EtNodeType::new_op_add(additive_expr_node).to_et_node()
            },
            Minus => {
                EtNodeType::new_op_sub( additive_expr_node).to_et_node()}
...
fn process_multiplicative_expr(et_tree: &mut EtTree, ast_tree: &AstTree, scope_tree: &ScopeTree, multiplicative_expr_node: u32, scope_node: u32, parent_et_node: u32) {
    ...
    let get_expr_node_of_op_node = |op_node_index| {
        ...
        match term_id!(at op_node in ast_tree) {
            Star => {
                EtNodeType::new_op_mul(multiplicative_expr_node).to_et_node()
            },
            Div => {
                EtNodeType::new_op_div( multiplicative_expr_node).to_et_node()
            },
            Mod => {
                EtNodeType::new_op_mod(multiplicative_expr_node).to_et_node()
            },
