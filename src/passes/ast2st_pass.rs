...
impl Pass for Ast2StPass {
    ...
    fn run(&mut self, ctx:&mut Context) -> Result<()> {
        ...
        if self.is_gen_png {
            ...
            generate_png_by_graph(&ctx.scope_tree.clone(), "scope_tree".to_string(), &[Config::EdgeNoLabel, Config::Record, Config::Title("scope_tree".to_string())],&mut ctx.io_task_list)?;
        }
        Ok(())
    }
    // 返回pass的描述，具体作用
